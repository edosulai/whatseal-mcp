...
export function hermesMcpAttachCommand(projectRoot) {
  const command = path.join(projectRoot, 'mcp-wrapper.sh');
  return {
    host: 'hermes',
    skillPath: `~/.hermes/skills/${DEFAULT_SKILL_NAME}/SKILL.md`,
    add: `printf 'Y\n' | hermes mcp add whatseal --command ${JSON.stringify(command)}`,
    config: {
      mcp_servers: {
        whatseal: {
          command,
    ...
    note: 'Restart Hermes after adding. Tools register as mcp_whatseal_whatsapp_*.',
  };
}
