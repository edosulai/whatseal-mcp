...
export function backendLifecycleCommands(accountId, projectRoot, { platform = process.platform } = {}) {
  ...
  return {
    install: reason,
    start: reason,
    status: accountId ? `node cli.mjs status --account ${accountId}` : 'node cli.mjs status',
    restart: reason,
    qrCli,
  };
}
...
export function buildReadinessGuidance({
  ...
  if (resolvedCode === 'UNKNOWN_ACCOUNT') {
    ...
    agentNextSteps.push(
      ...
      'If the background service was never installed on this machine, use install instead of start.',
