...
export function accountPaths(accountId = null) {
  const baseRoot = process.env.WHATSAPP_AGENT_ROOT || DEFAULT_ROOT;
  const baseState = process.env.WHATSAPP_AGENT_STATE || DEFAULT_STATE;
  ...
  return Object.freeze({
    ...
    socket: path.join(state, 'control.sock'),
    stateFile: path.join(state, 'status.json'),
    // Private per-account secrets (chat-lock / secret code, etc). Mode 0600.
    // Never commit; lives under ~/.local/state/whatsapp-agent/<account>/.
...
export function rpcCall(method, params = {}, { timeoutMs = 30000, socketPath = null } = {}) {
  return new Promise((resolve, reject) => {
    const request = JSON.stringify({ id: randomUUID(), method, params });
    const socket = net.createConnection(socketPath || paths.socket);
    let buffer = '';
    let settled = false;
...
export function backendLifecycleCommands(accountId, projectRoot) {
  const script = path.join(projectRoot || process.cwd(), 'install-launchagent.sh');
  const accountFlag = accountId ? ` --account ${accountId}` : '';
  return {
    install: `${script} install${accountFlag}`,
    start: `${script} start${accountFlag}`,
    status: `${script} status${accountFlag}`,
    restart: `${script} restart${accountFlag}`,
    qrCli: accountId
      ? `node cli.mjs qr --account ${accountId}`
      : 'node cli.mjs qr',
  };
}
...
export function buildReadinessGuidance({
  ...
  if (resolvedCode === 'UNKNOWN_ACCOUNT') {
    ...
    agentNextSteps.push(
      'Tell the user the backend is stopped and ask permission before starting it.',
      `Suggested command: ${commands.start}`,
      'If LaunchAgent was never installed on this machine, use install instead of start.',
      'After start, poll whatsapp_status or whatsapp_wait_ready until ready=true or pairing is required.',
    );
    userSteps.push(
    ...
    agentNextSteps.push(
      'Tell the user to wait for initial sync.',
      'Poll whatsapp_wait_ready for up to 60–120 seconds before declaring failure.',
    );
