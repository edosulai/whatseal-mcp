...
export function parseBrowserPolicy(env = process.env) {
  const raw = String(env.BROWSER_POLICY ?? env.WHATSAPP_BROWSER_POLICY ?? 'on_demand')
    .trim()
    .toLowerCase()
    .replace(/-/g, '_');
  if (raw === 'always' || raw === 'hot' || raw === 'ready') return 'always';
  if (raw === 'idle' || raw === 'warm') return 'idle';
  if (raw === 'on_demand' || raw === 'ondemand' || raw === 'demand' || raw === 'cold') {
    return 'on_demand';
  }
  // Default and unknown values: on_demand (Chrome stays down until first WA RPC).
  return 'on_demand';
}
...
export function methodNeedsBrowser(method) {
  ...
  // wake is intentionally not socket-only: dispatch runs ensureReady first.
