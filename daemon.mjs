...
//
// Browser memory policy (default: on_demand) — same contract as instaseal:
...
function syntheticRichAction(kind) {
  return getSyntheticRichAction(kind, { MessageMedia, Location });
}
...
async function resumeHotPathAfterLock(state) {
  ...
  logJson('info', 'lock-power-resume-restart', {
    ...
    screenLocked: Boolean(state?.screenLocked),
    lidClosed: Boolean(state?.lidClosed),
    sessionDaemon: !shouldExitProcessOnLockResume(),
  });
  pausedByLock = false;
  ...
  // LaunchAgent KeepAlive expects a clean process exit so launchd restarts us.
  // Session daemons have no KeepAlive — stay up and keep Chrome off unless
  // the policy is idle/always.
  if (shouldExitProcessOnLockResume()) {
  ...
  if (!shouldStartBrowserOnBoot(browserPolicy)) {
    ...
    chromeAlive = false;
    await publishState();
    logJson('info', 'lock-power-session-idle', {
      note: 'Session daemon stayed up; Chrome stays down until the next WA RPC',
      policy: browserPolicy,
    });
    return;
  }
...
async function executeApprovedAction(draft) {
  ...
  if (draft.action === 'send-rich') {
    const specification = syntheticRichAction(draft.payload?.kind);
...
async function dispatchCore(method, params = {}) {
  ...
  if (method === 'prepareRichTest') {
    ...
    const chat = await resolveChat(params.chat);
    const specification = syntheticRichAction(kind);
