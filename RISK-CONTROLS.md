...
| Third-party dependency compromise | Direct versions pinned; vendored `whatsapp-web.js@1.34.7` requires `puppeteer-core@24.38.0` (no browser postinstall) so `npx` does not download Chrome or chrome-headless-shell; published `npm-shrinkwrap.json` locks that tree; daemon uses system Chrome via `WHATSAPP_CHROME_PATH` or platform candidates; lockfile and integrity hashes are part of the approved tuple; `npm ci --ignore-scripts`; no automatic update | Lockfile SHA-256 drift gate; `npm audit`; source/resolved URL shown in compatibility report | Stop daemon, review diff/advisories, rebuild from reviewed lockfile | Registry, GitHub, npm, Chrome, and transitive packages remain external trust dependencies |
...
- `whatsapp-web.js` source, resolved registry URL, and integrity hash
